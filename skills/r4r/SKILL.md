...
4. Commit only intended ticket changes in each involved project that has a relevant diff.
5. Push each ticket branch.
6. Open or update a pull request for each involved project with:
   - a clear title and summary,
   - the issue link,
   - validation notes,
   - remaining risks or follow-up notes.
7. Move the ticket status to `In review` in the project board when project access is available.
8. End the response with a clear list of PR links, one per involved project.
